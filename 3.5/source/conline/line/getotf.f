      subroutine getotf(scanno, record, iptwh, ierr)
C-------------------------------------------------------------------------------
C  @(#)getotf.f	5.2 09/10/98
c
...
c
      if (online) then
c
         call gotfrec(is4, if4, lrec, nrec, xtemp, irtn)
c				nrec is set by gotfrec
         if (irtn .eq. 0) then
...
c			getotfrec had a problem, irtn is appropriate code
            ierr = irtn
            goto 99
         endif
      endif
