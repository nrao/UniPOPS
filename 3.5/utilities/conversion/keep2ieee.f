      program keep2ieee
c
c     @(#)keep2ieee.f	5.1 06/22/94
c
c     Reads in a record from presumably a Modcomp KEEP file and outputs a 
c     record in IEEE KEEP format.  Uses Standard Input and Standard
...
c
      data ltype/'r','c','i','c','b','d','i','r','i','r','i','r','i',
     1		 'r','d','c','d','r','i','b','r'/
...
c
	...
	if (ierr .eq. 0) then
	  ...
	  write(0,*) "KEEP2IEEE: Error on read"
	  goto 999
	endif
	nin = nin + 1
c	Read in a record
c
	...
	if (.not. isitcont(itype) .and. .not. isitline(itype)) then
	   write(0,*) "KEEP2IEEE: Bad scan type:", itype
	   goto 999
	endif
...
c	      telescope.
c
	      call cvtmodieee(cinbuff,coutbuff,ltype,nltype,ilnum)
	   else
	      call cvtmodieee(cinbuff,coutbuff,ctype,nctype,icnum)
	   endif
...
c
	   ...
	   if (ierr .ne. ibuffsize) then
		write(0,*) "KEEP2IEEE: Error on write"
		goto 999
	   endif
