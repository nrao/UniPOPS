      program ieee2keep
c
c     @(#)ieee2keep.f	5.1 06/22/94
c
c     Reads in a record from presumably a IEEE keep file and outputs a 
c     record in Modcomp KEEP format.  Uses Standard Input and Standard
...
c
      character*1 ltype(21), ctype(18), arg
      character*5120 cinbuff, coutbuff
      integer*4 nltype(21), nctype(18), iargc, icnum, ilnum, ibuffsize,
     .		ierr, long
      integer*2 iinbuff(2560), ioutbuff(2560), lsno, ktos, ktel, nin,
     .		itype, li
...
c
      data ltype/'r','c','i','c','s','d','i','r','i','r','i','r','i',
     1		 'r','d','c','d','r','i','s','r'/
      data nltype/2 , 12, 1 , 18,  2, 1 , 4 , 2 , 20, 23, 2 , 1 , 4 ,
     1            73, 8 , 72, 6 , 4 ,112, 28, 0 /
c
      data ctype/'r','i','c','i','c','s','d','i','r','i','r','i','r',
     1		 'i','r','i','r','r'/
      data nctype/1 , 2 , 12, 1 , 18,  2, 1 , 4 , 2 , 12, 1 , 8 , 22,
     1            2 , 1 , 4 , 16,1211/
c
      data icnum/18/, ilnum/21/
c
...
c
	...
	if (ierr .eq. 0) then
	  ...
	  write(0,*) "IEEE2KEEP: Error on read"
	  goto 999
	endif
	nin = nin + 1
c	Read in a record
c
	...
	if (.not. isitcont(itype) .and. .not. isitline(itype)) then
	   write(0,*) "IEEE2KEEP: Bad scan type:", itype
	   goto 999
	endif
...
c	      telescope.
c
	      call cvtieeemod(cinbuff,coutbuff,ltype,nltype,ilnum)
	   else
	      call cvtieeemod(cinbuff,coutbuff,ctype,nctype,icnum)
	   endif
c	   Converts from input IEEE binary to the binary format of the 
c	   Modcomp KEEP. 
c
	   call writebuff(ierr, ibuffsize, coutbuff)
	   if (ierr .ne. ibuffsize) then
		write(0,*) "IEEE2KEEP: Error on write"
		goto 999
	   endif
